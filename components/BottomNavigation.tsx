...
import { HomeIcon, QuizIcon, ProfileIcon, SettingsIcon, HeadphonesIcon } from './icons';

interface BottomNavigationProps {
  currentTab: 'home' | 'quiz' | 'podcast' | 'profile';
  onTabChange: (tab: 'home' | 'quiz' | 'podcast' | 'profile') => void;
  onOpenSettings: () => void;
}
    ...
    <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 pb-[env(safe-area-inset-bottom)] z-50 rounded-t-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
      <div className="flex justify-around items-center h-16 px-1">
        ...
        >
          ...
          <span className="text-[10px] sm:text-xs font-bold">Quiz</span>
        </button>
        
        <button 
          onClick={() => onTabChange('podcast')}
          className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors duration-200 group ${currentTab === 'podcast' ? 'text-primary-500' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
        >
          <div className={`p-1 rounded-full transition-transform duration-200 ${currentTab === 'podcast' ? 'bg-primary-100 dark:bg-primary-900/30 scale-110' : 'group-hover:scale-105'}`}>
            <HeadphonesIcon className="h-6 w-6" />
          </div>
          <span className="text-[10px] sm:text-xs font-bold">Podcast</span>
        </button>
